...
func fetchItemsByFilterOptionsPrice(stack : Int8, completion: @escaping ([ItemElement]?) -> Void) async {
    let urlString = "http://laksithanibm-001-site1.jtempurl.com/api/items/filter/price/\(stack)"
    guard let url = URL(string: urlString) else {
        print("Invalid URL: \(urlString)")
        completion(nil)
        return
